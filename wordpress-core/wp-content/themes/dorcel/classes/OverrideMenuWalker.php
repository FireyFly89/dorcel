...
/**
 * Override navigation Menu API: Walker_Nav_Menu class
 *
 *
 * @package SLS
 * @since 5.2.2
 */
class OverrideMenuWalker extends Walker_Nav_Menu
{
    ...
    <li id="menu-item-<?php echo $item_id; ?>" class="<?php echo implode(' ', $classes); ?>">
        ...
        <div class="menu-item-settings wp-clearfix" id="menu-item-settings-<?php echo $item_id; ?>">
            ...
            <?php if ('custom' == $item->type) : ?>
                <p class="description description-wide icon-dropdown">
                    <label for="edit-menu-item-icon-<?php echo $item_id; ?>">
                        <?php _e('Choose icon to display'); ?><br/>
                        <?php $select_icon_value = get_post_meta($item_id, '_menu_item_icon', true); ?>

                        <select id="edit-menu-item-icon-<?php echo $item_id; ?>" class="widefat edit-menu-item-icon"
                                name="menu-item-icon[<?php echo $item_id; ?>]">
                            <option value="" <?php echo !empty($select_icon_value) ? "" : "selected" ?>>No icon</option>

                            <?php foreach ($fontawesome_icon_list as $icon_family => $icons) : ?>
                                <?php echo sprintf('<option value="" disabled>%s:</option>', $icon_family); ?>

                                <?php foreach ($icons as $icon_name => $icon_entity) : ?>
                                    <?php echo sprintf('<option value="%s" %s>%s &#x%s;</option>', $icon_name, (!empty($select_icon_value) && $select_icon_value == $icon_name) ? "selected" : "", $icon_name, $icon_entity); ?>
                                <?php endforeach; ?>
                            <?php endforeach; ?>
                        </select>
                    </label>
            ...
            <?php if ('custom' == $item->type) : ?>
                <label for="edit-menu-item-under-desc-<?php echo $item_id; ?>">
                    <?php _e('Menu under-description'); ?><br/>
                    <?php $under_description_value = get_post_meta($item_id, '_menu_item_under_desc', true); ?>
                    <input type="text" id="edit-menu-item-under-desc-<?php echo $item_id; ?>"
                           class="widefat edit-menu-item-under-desc"
                           name="menu-item-under-desc[<?php echo $item_id; ?>]"
                           value="<?php echo esc_attr($under_description_value); ?>"/>
                </label>
            <?php endif; ?>
...
/*
 * Override the default Wordpress Nav Menu Walker
 */
add_filter('wp_edit_nav_menu_walker', function ($walker, $menu_id) {
    return 'OverrideMenuWalker';
}, 10, 2);
...
{
    if (array_key_exists('menu-item-icon', $_REQUEST) && is_array($_REQUEST['menu-item-icon']) && array_key_exists($menu_item_db_id, $_REQUEST['menu-item-icon'])) {
        $custom_value = $_REQUEST['menu-item-icon'][$menu_item_db_id];
        update_post_meta($menu_item_db_id, '_menu_item_icon', $custom_value);
    }

    if (array_key_exists('menu-item-under-desc', $_REQUEST) && is_array($_REQUEST['menu-item-under-desc']) && array_key_exists($menu_item_db_id, $_REQUEST['menu-item-under-desc'])) {
        $custom_value = $_REQUEST['menu-item-under-desc'][$menu_item_db_id];
        update_post_meta($menu_item_db_id, '_menu_item_under_desc', $custom_value);
    }
}

/*
 * Adds value of new field to $item object that will be passed to OverrideMenuWalker
 */
add_filter('wp_setup_nav_menu_item', 'custom_nav_item');
function custom_nav_item($menu_item)
{
    $menu_item->icon = get_post_meta($menu_item->ID, '_menu_item_icon', true);
    $menu_item->under_desc = get_post_meta($menu_item->ID, '_menu_item_under_desc', true);
    return $menu_item;
}
